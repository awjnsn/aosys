...
{
    fprintf(stderr,
            "usage: %s <threads> <regular|lock|getcpu-atomic|rseq-atomic|rseq> "
            "[rounds]\n",
            argv0);
    exit(EXIT_FAILURE);
}
...
int main(int argc, char *argv[])
{
    ...
    if (!strcmp(MODE, "rseq"))
        args.operation = operation_rseq;
    else if (!strcmp(MODE, "getcpu-atomic"))
        args.operation = operation_atomic;
    else if (!strcmp(MODE, "rseq-atomic"))
        args.operation = operation_rseq_atomic;
    else if (!strcmp(MODE, "regular"))
        args.operation = operation_regular;
    else if (!strcmp(MODE, "lock"))
        args.operation = operation_lock;
    else
        usage(argv[0]);
    ...
    args.counters = aligned_alloc(sizeof(struct cacheline),
                                  CPUS * sizeof(struct cacheline));
    if (!args.counters)
        die("calloc");
    ...
    printf("mode=%s threads=%d sum=%ld state=%s aborts=%ld cputime=%fs "
           "per_increment=%fns\n",
           MODE, NTHREADS, sum, (sum % ROUNDS_PER_THREAD) == 0 ? "ok" : "fail",
           aborts,
